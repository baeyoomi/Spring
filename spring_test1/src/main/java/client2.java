...
class client_chat {
	private final String ip= "192.168.110.217";	//서버IP
	...
	public void chat() {
		try {
			...
			while(true) {		//접속 유지
				...
				if(ck == 0) {	//최초 서버 접속시 server에 입장 메세지 출력
					this.msg = "["+this.mid+"] : 님 입장하였습니다.";
					
				}else {			//사용자가 입력하는 메세지
					this.sc = new Scanner(System.in);
					System.out.println("client 메세지 내용을 입력해 주세요 : ");
					this.check = this.sc.nextLine().intern();
					this.msg = "["+this.mid+"] : " + this.check;	//서버로 발송할 문구
				}
				if(this.check == "exit") {	//메세지에 exit 입력시 접속 종료
